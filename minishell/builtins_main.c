...
void    ft_execute_builtins(char **commande, char **envp)
{

    static char **nos_variables = NULL;

    if ((ft_strncmp(commande[0], "echo", 4)) == 0)
        ft_echo(&(commande[0]));
    if (ft_strncmp(commande[0], "export", 6) == 0)
        nos_variables = ft_export(commande, envp, nos_variables);
//    if (strncmp(cmd_test[0], "env", 3) == 0)
//        ft_env(envp, &(args_in));
//    if (ft_strncmp(cmd_test[0], "cd", 2) == 0)
//        ft_cd(cmd_test, envp);
//    if (ft_strncmp(cmd_test[0], "unset", 5) == 0)
//          ft_unset(cmd_test, &(args_in));
